...
export function CardMedia({ item, locale, position }: { item: SiteItem; locale: Locale; position: "top" | "bottom" }) {
  ...
  return <><button type="button" className={`card-media card-media-${item.media_size || "medium"} ${isPdf ? "pdf" : "image"}`} onClick={() => setOpen(true)} aria-label={title}>
    {isPdf ? <><FileText size={34} aria-hidden="true" /><span><small>PDF</small><b>{title}</b><em>{locale === "he" ? "צפייה במסמך" : "عرض المستند"}</em></span></> : <img src={url} alt={title} loading="lazy" style={{ objectFit: item.media_fit || "cover" }} />}
...
function MediaViewer({ url, title, isPdf, locale, onClose }: { url: string; title: string; isPdf: boolean; locale: Locale; onClose: () => void }) {
  ...
  useEffect(() => { setMounted(true); const handle = (event: KeyboardEvent) => { if (event.key === "Escape") onClose(); }; window.addEventListener("keydown", handle); const original = document.body.style.overflow; document.body.style.overflow = "hidden"; return () => { window.removeEventListener("keydown", handle); document.body.style.overflow = original; }; }, [onClose]);
